...
const Header = ({ page, darkMode, setDarkMode, showDropdownPage, setShowDropdownPage, resultsVisible, setResultsVisible }) => {
        ...
        const handleGlobalKeyDown = (event) => {
            ...
            if (resultsVisible) {
                if (event.key === 'Escape') {
                    event.preventDefault();
                    setResultsVisible(false);
                    document.querySelectorAll('.search-container-items')[0].focus();
                    return;
                }
                if (event.key === 'Tab') {
    ...
    // Ref to hold the cleanup function for the search block
    const searchBlockCleanupRef = useRef(null);

    useEffect(() => {
        if (query !== '' && query.length > 0 && resultsVisible) {
            // Clean up previous listeners if any
            if (searchBlockCleanupRef.current) {
                searchBlockCleanupRef.current();
                searchBlockCleanupRef.current = null;
            }
            searchBlockCleanupRef.current = Block.makeBlockAccessible('inner-body-div', 'search-container-items');
        } else {
            // Clean up when search is hidden
            if (searchBlockCleanupRef.current) {
                searchBlockCleanupRef.current();
                searchBlockCleanupRef.current = null;
            }
    ...
    return (
        <div className="header">
            ...
            <form className="header-search-div" role='search'>
                ...
                <textarea ref={searchInputRef} type="text" placeholder="Search" className='block-interactive search-container-items resize-none h-6' aria-label='Search' value={query} onChange={(event) => setQuery(event.target.value)} />
            ...
            <div className='header-buttons-grid-div' id="header-buttons-grid-div">
                ...
                <button className='header-button block-interactive center-flex' onMouseDown={() => { setDarkMode((prevDarkMode) => !prevDarkMode) }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === " ") { event.preventDefault(); event.stopPropagation(); setDarkMode((prevDarkMode) => !prevDarkMode) } }} aria-label='Toggle dark mode'>
                    <img src={sunicon} alt='Sun Icon' id='Sun' className='theme-mode-image' aria-hidden="true"></img>
                </button>
            </div>
            ...
            {query !== '' && query.length > 0 && resultsVisible && (
                <div className='search-result-container-overlay fixed top-[56px] left-0 right-0 bottom-0 animate-fadeIn'>
                    <div aria-live='polite' id='search-container' className='search-result-container fixed max-h-[350px] rounded-md z-[999] m-auto left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 slideDown' tabIndex="-1" role="dialog" aria-modal="true" aria-label="Search results">
                        <div className='flex justify-end bg-transparent p-2 search-close-button-div'>
                            <button className='search-close-button search-container-items rounded-md p-1' onClick={() => { setResultsVisible(false) }} aria-label='Close search result modal'>
