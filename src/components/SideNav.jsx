    ...
    <Col xs={0} sm={0} md={0} lg={3} className='px-0'>
      <div className='side-nav-div overflow-y-auto flex flex-col sticky h-[calc(100vh-56px)]' id='side-nav-div'>
        ...
        <div className='side-nav-links-section'>
          ...
          <Link onClick={() => {sessionStorage.setItem(`scroll-position-${page}`, window.scrollY)}} to='/examples/tabs' aria-label="View tabs examples" className={`side-nav-link ${(page === 'tabs') ? 'active-nav-link' : ''} ${(window.innerWidth >= 992) ? 'block-interactive' : ''}`} style={{marginTop: '12px'}}>Tabs</Link>
